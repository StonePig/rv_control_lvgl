...
extern "C" void camera_bridge_request_start(int index)
{
    ...
    jmethodID mid = env->GetStaticMethodID(cls, "startCamera", "(I)V");
    if (!mid) {
        __android_log_print(ANDROID_LOG_ERROR, "camera_bridge", "startCamera method not found");
        return;
    }
    env->CallStaticVoidMethod(cls, mid, (jint)index);
}
...
extern "C" void camera_bridge_request_stop(int index)
{
    ...
    jclass cls = env->FindClass("com/hybird/lvgl/android/MainActivity");
    if (!cls) return;
    jmethodID mid = env->GetStaticMethodID(cls, "stopCamera", "(I)V");
    if (!mid) return;
    env->CallStaticVoidMethod(cls, mid, (jint)index);
}
